...
// Copyright (c) 2014 The Singularity developers
...
const string strMessageMagic = "Singularity Signed Message:\n";
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("SingularityMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("SingularityMiner : ProcessBlock, block not accepted");
    }
...
{
    printf("Singularity miner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    ...
    {
        printf("Singularity miner terminated\n");
        throw;
    }
}
