...
void ThreadMapPort()
{
    ...
    if (r == 1)
    {
        ...
        string strDesc = "Singularity " + FormatFullVersion();
...
// hidden service seeds
 static const char *strNetOnionSeed[][1] = {
     {"bmzfxelpwpiboixp.onion"},
     {"uu7g4qx6jqd6pzzp.onion"},
     {NULL}
  };
...
bool BindListenPort(const CService &addrBind, string& strError)
{
    ...
    if (::bind(hListenSocket, (struct sockaddr*)&sockaddr, len) == SOCKET_ERROR)
    {
        ...
        if (nErr == WSAEADDRINUSE)
            strError = strprintf(_("Unable to bind to %s on this computer. Singularity is probably already running."), addrBind.ToString().c_str());
        else
